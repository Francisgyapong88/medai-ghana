...
export default function Patients({ onNavigate }: PatientsProps) {
  ...
  const loadPatientDetails = (patientId: number) => {
    ...
    axios.get('http://localhost:5000/api/allergies/options', { headers })
      .then(res => setAllergyOptions(res.data.data))
      .catch(err => console.error('Failed to load allergy options:', err))

    axios.get(`http://localhost:5000/api/allergies/patient/${patientId}`, { headers })
      .then(res => setPatientAllergies(res.data.data))
      .catch(err => console.error('Failed to load patient allergies:', err))

    axios.get(`http://localhost:5000/api/medical-history/patient/${patientId}`, { headers })
  ...
  const openPatient = async (patient: Patient) => {
    ...
    try {
      ...
      const res = await axios.get(`http://localhost:5000/api/patients/${patient.patient_id}`, { headers })
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:5000/api/patients/${selectedPatient.patient_id}`,
        {
  ...
  const handleDelete = async (patient: Patient) => {
    ...
    try {
      ...
      await axios.delete(`http://localhost:5000/api/patients/${patient.patient_id}`, { headers })
  ...
  const handleAddAllergy = async () => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5000/api/allergies/patient/${selectedPatient.patient_id}`,
        {
  ...
  const handleAddCondition = async () => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5000/api/medical-history/patient/${selectedPatient.patient_id}`,
        {
